...
def func(x,v):
    return np.cos(x)**3.-3.*np.cos(x)+4*v-2
...
while i<M:
    ...
    k=0
    while k<len(theta):
        theta[k]=sc.optimize.fsolve(func, pi/2, args=(a2[k]))#, fprime=None, full_output=0, col_deriv=0, xtol=1.49012e-08, maxfev=0, band=None, epsfcn=None, factor=100, diag=None)
        k=k+1
# sc.optimize.fsolve(func, 1.01*pi, args=(1.))
...
plt.step(xfi,Hfi,'c',alpha = 0.5,label=r'$\langle\phi\rangle=$'+'{:.{}f}'.format(SHfi[M-1], 3 ))
plt.xlabel(r'$ \phi $')
plt.ylabel('n')
plt.legend(loc='best')
F10=plt.subplot(2, 2, 2 )  
plt.step(xth,Hth,'m',alpha = 0.5,label=r'$\langle\vartheta\rangle=$'+'{:.{}f}'.format(SHth[M-1], 3 ) )
plt.xlabel(r'$ \vartheta $')
plt.xlim([0,pi])
plt.ylabel('n')
plt.legend(loc='best')
F10=plt.subplot(2, 2, 3 )  
plt.step(xCth,HCth,'y',alpha = 0.5,label=r'$\langle\cos(\vartheta)\rangle=$'+'{:.{}f}'.format(SHCth[M-1], 4 ) )
plt.xlabel(r'$ \cos(\vartheta) $')
plt.ylabel('n')
plt.legend(loc='best')
F10=plt.subplot(2, 2, 4 )  
plt.step(xC2th,HC2th,'k',alpha = 0.5,label=r'$\langle\cos^2(\vartheta)\rangle=$'+'{:.{}f}'.format(SHC2th[M-1], 4 ) )
plt.xlabel(r'$ \cos^2(\vartheta) $')
plt.ylabel('n')
plt.legend(loc='best')
plt.suptitle('Normirani histogrami porazdelitev generiranega dipolnega sevanja in nekaterih momentov '+'(N='+str(N)+')',fontsize=16)

F20=plt.figure(20)
F20=plt.subplot(3, 2, 1 )  
plt.step(xh01,Hh01,'y',alpha = 0.5,label=r'$\langle Y^0_1 \rangle=$'+'{:.{}f}'.format(SH01[M-1], 6 ) )
plt.xlabel(r'$ Y^0_1  $')
plt.ylabel('n')
plt.legend(loc='best')
F20=plt.subplot(3, 2, 2 )  
plt.step(xh11,Hh11,'c',alpha = 0.5,label=r'$\langle Y^1_1 \rangle=$'+'{:.{}f}'.format(SH11[M-1], 6 ))
plt.xlabel(r'$  Y^1_1 $')
plt.ylabel('n')
plt.legend(loc='best')
F20=plt.subplot(3, 2, 3 )  
plt.step(xh02,Hh02,'k',alpha = 0.5,label=r'$\langle Y^0_2 \rangle=$'+'{:.{}f}'.format(SH02[M-1], 6 ))
plt.xlabel(r'$  Y^0_2 $')
plt.ylabel('n')
plt.legend(loc='best')
F20=plt.subplot(3, 2, 4 )  
plt.step(xh12,Hh12,'g',alpha = 0.5,label=r'$\langle Y^1_2 \rangle=$'+'{:.{}f}'.format(SH12[M-1], 6 ))
plt.xlabel(r'$  Y^1_2  $')
plt.ylabel('n')
plt.legend(loc='best')
F20=plt.subplot(3, 2, 5 )  
plt.step(xh22,Hh22,'r',alpha = 0.5,label=r'$\langle Y^2_2 \rangle=$'+'{:.{}f}'.format(SH22[M-1], 6 ))
plt.xlabel(r'$ Y^2_2 $')
plt.ylabel('n')
plt.legend(loc='best')
F20=plt.subplot(3, 2, 6 )  
plt.step(xh03,Hh03,'b',alpha = 0.5,label=r'$\langle Y^0_3 \rangle=$'+'{:.{}f}'.format(SH03[M-1], 6 ))
plt.xlabel(r'$ Y^0_3 $')
plt.ylabel('n')
plt.legend(loc='best')
plt.suptitle('Normirani histogrami porazdelitev momentov greneriranega dipolnega sevanja '+'(N='+str(N)+')',fontsize=16)
...
plt.plot(n,abs(SHC2th-1./5.),'k',alpha = 0.5,label=r'$\langle\cos^2(\vartheta)\rangle-1/5$')#+'{:.{}f}'.format(SHfi, 3 ))
plt.xscale('log')
plt.yscale('log')
plt.xlabel(' N ')
plt.title('Absolutne vrednosti momentov generiranega dipolnega sevanja za različno velike vzorce')#+'(N='+str(N)+',M='+str(M)+')')
plt.legend(loc=1)
...
plt.plot(n,abs(SH02 + 1/(20.*pi)**(0.5)),'k',alpha = 0.5,label=r'$\langle  Y^0_2 \rangle + 1/2\sqrt{5 \pi}$')#+'{:.{}f}'.format(SHth, 3 ) )
