...
def Fun1(zacetni,yz):  ## prosti končni robni pogoj
#    tol=10**(-5)
    l0=1
    vmin=0.8
    vmax=1.2
...
#    exp2=sum(e**(zacetni[1:-2]*dx) - yz/2*dx - zacetni[-2]/2*dx +l0-tol)  ## kaznuj odstopanje od   integral(v*dt)=pot
    ...
    for i in zacetni[0:-1]:
        V_MIN=V_MIN+(e**(10*( vmin-i)))    ## kaznuje premajhne (negativne) hitrosti
        V_MAX=V_MAX+(e**(10*( i-vmax)))    ## kaznuj prevelike hitrosti
#        if i<vmin:
#            V_MIN=V_MIN+(e**(( vmin-i)))    ## kaznuje premajhne (negativne) hitrosti
#        if i>vmax:
#            V_MAX=V_MAX+(e**((-vmax+i)))    ## kaznuj prevelike hitrosti
...
def Fun2(zacetni,yz,yk):  ## prosti končni robni pogoj
#    tol=10**(-5)
    l0=1
    vmin=0.8
    vmax=1.2
...
#    dx=1/(len(zacetni)-1)
#    exp1=sum(e**(zacetni[1:-2]*dx) + yz/2*dx + zacetni[-2]/2*dx -l0)       ## kaznuje odstopanje od  integral(v*dt)=pot
#    exp2=sum(e**(zacetni[1:-2]*dx) - yz/2*dx - zacetni[-2]/2*dx +l0-tol)  ## kaznuje odstopanje od   integral(v*dt)=pot
    
    V_MAX=0
    V_MIN=0
    for i in zacetni[0:-1]:
        if i<vmin:
            V_MIN=V_MIN+(e**(1*( vmin-i)))    ## kaznuje premajhne (negativne) hitrosti
        if i>vmax:
            V_MAX=V_MAX+(e**(1*(-vmax+i)))    ## kaznuje prevelike hitrosti
...
#    plt.plot(x,res1.x, color=barva[c], LS=':',alpha = 0.95,  label=r'$y_0$={}'.format(yz))
    ...
    res11 = minimize(Fun1, np.append(x,lagMult), args=(yz), method='L-BFGS-B',tol=1e-10,options={'maxiter': 5000})   ## minimizacija za prosti robni pogoj
...
#    res11.x[0]=yz ## popravim začetni pogoj
    plt.plot(x,res11.x[0:-1], color=barva[c], LS='',marker='x',alpha = 0.95,  label=r'$y_n$={}'.format(yz))
...
##    plt.plot(x,res1.x, color=barva[c], LS=':',alpha = 0.95,  label=r'$y_0$={}'.format(yz))
#    lagMult=6*(1-yz) ## začetni približek za lagrangejev multiplikator
#    res5 = minimize(Fun2, np.append(x,lagMult), args=(yz,yz), method='L-BFGS-B',tol=1e-10,options={'maxiter': 5000})   ## minimizacija za prosti robni pogoj
##    res11 = minimize(Fun1, np.append(x,lagMult), args=(yz), method='Powell',tol=1e-6)   ## minimizacija za prosti robni pogoj
#    print('success='+ str(res5.success))
##    res11.x[0]=yz ## popravim začetni pogoj
#    plt.plot(x,res5.x[0:-1], color=barva[c], LS='',marker='x',alpha = 0.95,  label=r'$y_n$={}'.format(yz))
#    plt.xlabel(r'$x$',fontsize=16)
#    plt.ylabel(r'$y(x)$',fontsize=16)
#    plt.xlim([0,1.2])
##    plt.legend(loc='best')
...
#        lagMult=6*(1-yz) ## začetni približek za lagrangejev multiplikator
#        res2 = minimize(Fun2, np.append(x,lagMult), args=(yz,yk), method='L-BFGS-B',tol=1e-10,options={'maxiter': 5000})   ## minimizacija za prosti robni pogoj
##        plt.plot(x,res2.x, color=barva[c],ls=':', alpha = 0.95, label=r'$y_0$={}'.format(yz)+r',  $y_T$={}'.format(yk))
#        print('success='+ str(res11.success))
#    #    res11.x[0]=yz ## popravim začetni pogoj
#        plt.plot(x,res2.x[0:-1], color=barva[c], LS='',marker=MARK[d],alpha = 0.65, label=r'$y_0$={}'.format(yz)+r',  $y_T$={}'.format(yk))
#        plt.xlabel(r'$x$',fontsize=16)
#        plt.ylabel(r'$y(x)$',fontsize=16)
#        plt.xlim([0,1.2])
#    
...
#d=1
#g=0
#for p in [1,2,5]: # števila polov
#    
##    plt.title(r'Vožnja do semaforja za prosti robni pogoj pri potenci '+r'$p$={}'.format(p),fontsize=16)
#
#    c=0
#    for yz in [0,0.5,1.,1.5]: # števila polov
#        F11=plt.subplot(3, 2, d ) 
#        plt.plot(x,fun3(x,yz,p), color=barva[c], alpha = 0.95, label=r'$y_0$={}'.format(yz) )
#        lagMult=6*(1-yz) ## začetni približek za lagrangejev multiplikator
#        res3 = minimize(Fun3, np.append(x,lagMult), args=(yz,p), method='L-BFGS-B',tol=1e-10,options={'maxiter': 5000})   ## minimizacija za prosti robni pogoj
##        plt.plot(x,res2.x, color=barva[c],ls=':', alpha = 0.95, label=r'$y_0$={}'.format(yz)+r',  $y_T$={}'.format(yk))
#        print('success='+ str(res3.success))
#    #    res11.x[0]=yz ## popravim začetni pogoj
#        plt.plot(x,res3.x[0:-1], color=barva[c], LS='',marker='x',alpha = 0.95, label=r'$y_0$={}'.format(yz)+r',  $p$={}'.format(p))
#        plt.xlabel(r'$x$',fontsize=16)
#        plt.ylabel(r'$y(x)$',fontsize=16)
#        plt.xlim([0,1.2])
#    
#    #    plt.legend(loc='best')
#    
#        F11=plt.subplot(3, 2, d+1 )
#        plt.plot(x,abs(res3.x[0:-1]-fun3(x,yz,p)), color=barva[c],ls=LS[g] ,alpha = 0.75,   label=r'$y_0$={}'.format(yz)+r',  $p$={}'.format(p))
#        print(d)
#        c=c+1
#    d=d+2
#    g=g+1
...
##    res33 = minimize(fun33, x, args=yz ,method='Nelder-Mead')
##    plt.plot(x,res33.x,  color=barva[c] , alpha = 0.95, label=r'$y_0$={}'.format(yz)+r',  ')
#    c=c+1
#    
#plt.xlabel(r'$x$',fontsize=16)
#plt.ylabel(r'$y(x)$',fontsize=16)
...
##
#F111=plt.figure(111)
#x=np.linspace(0,1,20)#-len(FTSIG)/2
#
#
#d=1
#for mu in [1,5,10]: # števila polov
##    plt.title(r'Vožnja do semaforja za prosti robni pogoj pri '+r'$\mu$={}'.format(mu),fontsize=16)
#
#    c=0
#    for yz in [0,0.5,1.,1.5]: # števila polov
#        F111=plt.subplot(3, 2, d ) 
#        plt.plot(x,fun4(x,yz,mu), color=barva[c], alpha = 0.95, label=r'$y_0$={}'.format(yz)+r',  $\mu$={}'.format(mu))
##        res4 = minimize(fun4, x, args=(yz,mu),method='Nelder-Mead')
##        plt.plot(x,res4.x, color=barva[c], alpha = 0.95, label=r'$y_0$={}'.format(yz) )
#        lagMult=6*(1-yz) ## začetni približek za lagrangejev multiplikator
#        res4 = minimize(Fun4, np.append(x,lagMult), args=(yz,mu), method='L-BFGS-B',tol=1e-10,options={'maxiter': 5000})   ## minimizacija za prosti robni pogoj
#    #    res11 = minimize(Fun1, np.append(x,lagMult), args=(yz), method='Powell',tol=1e-6)   ## minimizacija za prosti robni pogoj
#        print('success='+ str(res11.success))
#    #    res11.x[0]=yz ## popravim začetni pogoj
#        plt.plot(x,res4.x[0:-1], color=barva[c], LS='',marker='x',alpha = 0.95,   label=r'$y_0$={}'.format(yz)+r',  $\mu$={}'.format(mu))
#        plt.xlabel(r'$x$',fontsize=16)
#        plt.ylabel(r'$y(x)$',fontsize=16)
#    #    plt.legend(loc='best')
#    
#        F111=plt.subplot(3, 2, d+1 ) 
#        plt.plot(x,abs(res4.x[0:-1]-fun4(x,yz,mu)), color=barva[c], LS='-',alpha = 0.75,  label=r'$y_0$={}'.format(yz)+r',  $\mu$={}'.format(mu))
#        plt.xlabel(r'$x$',fontsize=16)
#        plt.ylabel(r'$|y_{a}(x)-y_{n}(x)|$',fontsize=16)
#        plt.xlim([0,1.2])
#        plt.yscale('log')
