...
def Hdata(dogodki): # vse dogodke popredalčka in normalizira
    H, binEdge = np.histogram(dogodki, bins=40,normed=True)
...
F10=plt.figure(10)
plt.suptitle(r'Povprečni čas izumrtja v odvisnosti od $dt$ ',fontsize=16)
plt.title(r'(računano za N='+str(N)+' ponovitev)')#+'(N='+str(N)+',M='+str(M)+')')
...
#    dt=0.01
    ...
    #    N0=25
        ...
        while  Zt>0 and Lt>0:
            ...
            dZ1=np.random.poisson(lam=5.0*Zt*dt, size=None) #rojstvo
            dL1=np.random.poisson(lam=4.0*Lt*dt, size=None)         #rojstvo
            dZ2=np.random.poisson(lam=4.0*Zt*dt, size=None)  #smrt
            dL2=np.random.poisson(lam=5.0*Lt*dt, size=None)        #smrt
            dZ3=np.random.poisson(lam=0.02*Zt*Lt*dt, size=None) #smrt
            dL3=np.random.poisson(lam=0.005*Zt*Lt*dt, size=None) #rojstvo
    ...
    Hkoc, xkoc, binEdge_koc, Lkoc=Hdata(T)
...
#    plt.step(xkoc,Hkoc,crta[k],alpha = 0.5,label=r'$dt=$ '+str(dT[k])+r';  $\langle t_{izumrtje} \rangle=$'+'{:.{}f}'.format(avg[k], 5 ))
plt.plot(dT,avg,'r',alpha = 0.95)#,label=r'$N_0=$'+str(N0))#+'{:.{}f}'.format(SHfi, 3 ))
plt.xlabel(r'$  dt$',fontsize=18)
plt.ylabel(r'$\langle t_{izumrtje} \rangle$',fontsize=18)
#plt.xlim([0,10])
plt.legend(loc='best')
...
#plt.step(xkoc,Hkoc,'y',alpha = 0.5,label=r'$\langle t_{izumrtje} \rangle=$'+'{:.{}f}'.format(avg[0], 5 ))
#plt.xlabel(r'$  t_{izumrtje} $')
#plt.ylabel('n')
#plt.legend(loc='best')
#plt.title(r'$\beta=1,\ N_0=25$, N='+str(N)+' ponovitev')#+'(N='+str(N)+',M='+str(M)+')')
