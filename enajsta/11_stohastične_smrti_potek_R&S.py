...
#    N0=25
    ...
    while  Nt[i]>0 :
        ...
        dNR=np.random.poisson(lam=4*beta*Nt[i]*dt, size=None)
        dNS=np.random.poisson(lam=5*beta*Nt[i]*dt, size=None)

        Nt=np.append(Nt, Nt[i]-dNS+dNR)
    ...
    F50=plt.figure(50)
    plt.suptitle(r'Umiranje populacije računano z različnimi časovnimi koraki; $N_0=250$,  $\beta_r=4$, $\beta_s=5$',fontsize=16)
    F50=plt.subplot(1, 2, 1 ) 
    plt.plot(T,Nt,crta[j],alpha = 0.95,label=r'$\Delta t=$'+str(dt))#+'{:.{}f}'.format(SHfi, 3 ))
#    plt.xscale('log')
#    plt.yscale('log')
    plt.ylabel(' N(t) ')   
    plt.xlabel(' t ')
