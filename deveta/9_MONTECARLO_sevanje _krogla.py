...
MU=np.empty(50) 
for k in range(0,50): MU[k]=(k+1)/5
...
for i in range(0,N):
    ...
    rN=np.append(rN,-r[i]*np.cos(theta[i])+np.sqrt(1-(r[i]*np.sin(theta[i]))**2)) 
    rkr=np.append(rkr,-np.log(1-y[i])/mu0) # prepotovana pot
   
    if rkr[i]>=rN[i]:     
        pobegli=pobegli+1

    for mu in range(0,50):
        
        Rkr=-np.log(1-y[i])/ ((mu+0.001)/5) # prepotovana pot

        if Rkr>=rN[i]:
            POBEGLI[mu]=POBEGLI[mu]+1
   
    if i==10**(m+1)-1:     #integrali oziroma vsota vseh vrednosti normaliziranega histograma => povpreča 
    
        Htelo, xtelo, binEdge_telo, Ltelo=Hdata(rN)  
        Hkrog, xkrog, binEdge_krog, Lkrog=Hdata(rkr)  
...
plt.xlim(0,6)
plt.legend(loc='best')
plt.title(r'Porazdelitev dolžin ubežnih in prostih poti za N='+str(N)+' fotonov')#+'(N='+str(N)+',M='+str(M)+')')

F20=plt.figure(20)
plt.plot(MU,POBEGLI/N,'k',alpha = 0.95)#,label=r'$kocka$')#+'{:.{}f}'.format(SHfi, 3 ))
plt.xlabel(r'$\mu$')
#plt.ylabel(r'$dP/dl$')
plt.ylabel(r'$P_{\mu}$')
#plt.xlim(0,6)
plt.legend(loc='best')
plt.title(r'Verjetnosti za pobeg fotonov računane za N='+str(N))#+'(N='+str(N)+',M='+str(M)+')')
